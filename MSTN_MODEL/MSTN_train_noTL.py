...
def mstn_trainmodel_noTL(TRAINING_FILE, VAL_FILE, val_file_num=100, epochs_limit=2500, step_log=True, model_name='mstn'):
    ...
    dis_positive, dis_negative, sc, tc, smoothness = model.adloss(x, xt, y, adlamb)
    ...
    train_writer = tf.summary.FileWriter('./MSTN_MODEL/MSTN_train_log/tensorboard'+MODEL_NAME)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(FLAGS.num_epochs):
            ...
            while step < train_batches_per_epoch_source:
                ...
                if gd % 20 == 0 and step_log==True:
                    ...
                    batch_x_target, batch_y_target = val_preprocessor_target.next_batch(val_file_num)
                    batch_x_source, batch_y_source = val_preprocessor_source.next_batch(200)
                    ...
                    draw_feature_graph(_feature_source, _feature_target, _result_source, _result_graph_sub, _result_graph_total, './MSTN_MODEL/MSTN_train_log/' + train_dir_name , gd)
        ...
        last_model_path = './MSTN_MODEL/MSTN_train_log/mstnmodel_mstn/' + \
             model_name + str(gd)+'.ckpt'
            ...
            while step < train_batches_per_epoch_source:
                ...
                if gd % 1 == 0:
                    ...
                    for batch_start_num in range(1, val_file_num, batch_max):
                        ...
                        batch_x_target, batch_y_target = val_preprocessor_target.next_batch(batch_num_current)
                        batch_x_source, batch_y_source = val_preprocessor_source.next_batch(200)
                        ...
                        log_feature_low(_feature_source, _feature_target, _result_source, _result_graph_total, savedir='./MSTN_MODEL/MSTN_train_log/' + train_dir_name + '/', logname=str(batch_start_num))
                        
                        print("Validation %.2f %% ..." %(100*val_preprocessor_target.pointer/val_file_num))
                    ...
                    result_graph_total = np.array([i for item in result_graph_total for i in item])
...
def log_feature_low(feature_source, feature_target, result_source, result_target_total, savedir, logname):
    if not os.path.exists(savedir):
        os.makedirs(savedir)
    ...
    #np.savetxt(fname=savedir + logname + "source.txt", X=feature_source_low)
    #np.savetxt(fname=savedir + logname + "target.txt", X=feature_target_low)

    fig = plt.figure()
    plt.plot(feature_source_low[:,0], feature_source_low[:,1], "ro")    # source domain: red
    plt.plot(feature_target_low[:,0], feature_target_low[:,1], "bo")    # target domain: blue
