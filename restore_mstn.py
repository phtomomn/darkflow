...
from mstnmodel.mstnmodel import AlexNetModel
from mstnmodel.preprocessor import BatchPreprocessor
...
tf.app.flags.DEFINE_string(
    'train_root_dir', './mstnmodel/training', 'Root directory to put the training data')
tf.app.flags.DEFINE_integer(
    'log_step', 10000, 'Logging period in terms of iteration')

NUM_CLASSES = 31
TRAINING_FILE = './mstnmodel/source.txt'
...
def mstn_label_with_model(MODEL_PATH=MODEL_PATH):
    ...
    train_op = model.optimize(
        decay_learning_rate, train_layers, adlamb, sc, tc)
    ...
    Ttrain_preprocessor = BatchPreprocessor(dataset_file_path=VAL_FILE, num_classes=NUM_CLASSES,
                                            output_size=[227, 227], horizontal_flip=True, shuffle=True, multi_scale=multi_scale)
    val_preprocessor = BatchPreprocessor(dataset_file_path=VAL_FILE, num_classes=NUM_CLASSES, output_size=[
            ...
            while step < train_batches_per_epoch:
                ...
                if gd % 1 == 0:
                    ...
                    for _ in range((len(val_preprocessor.labels))):
                        ...
                        """
                        acc = sess.run(correct, feed_dict={
                                       x: batch_tx, y: batch_ty, dropout_keep_prob: 1.})
                        test_acc += acc
                        test_count += 1
                        """
                        temp = sess.run(correct_test, feed_dict={
                                       x: batch_tx, y: batch_ty, dropout_keep_prob: 1.})
                        value_predict = temp // 100
                        value_real = temp % 100
                        print("figure%d\tpredict=%d\treal=%d"%(test_count+1, value_predict, value_real), end="")
                        if value_predict == value_real:
                            print("")
                            test_acc += 1
                        else:
                            print("\tERROR")
...
def main(_):
    mstn_label_with_model(MODEL_PATH_HERE)
